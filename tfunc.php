...
function acct_update($post) {
    ...
    $db->close();
    $url = "http://" . $_SERVER['HTTP_HOST'] . "/cart.php?admin=3";
...
function product_update($post) {
    ...
    $url = "http://" . $_SERVER['HTTP_HOST'] . "/cart.php?admin=4"; // After the query runs, it redirects you to the product list admin area.
    header("Location: " . $url) or die("Didn't work");
}
...
function admin_purchases_display() {
    ...
    $purchases_display .= '</tbody></table></div><!-- end div.purchases_display-->
     <div class="order_display_form">
       <form  class="account_edit_form" method="POST" action="cart.php?admin=5&order=1">
...
function add_to_cart($productId,$quantity){
    ...
    ob_clean();
    $url = "http://petertwickler.com/cart.php";
...
if (isset($_GET['prod_name']) && $_GET['prod_name'] != 1) {
    ...
    if ($_SESSION['sign_in'] == 1) {
        ...
        $url = "http://" . $_SERVER['HTTP_HOST'] . "/cart.php";
        header("Location: " . $url) or die("Didn't bb work");
    }

    else {
        ob_clean();
        $url = "http://" . $_SERVER['HTTP_HOST'] . "/cart.php?signed=0";
        header("Location: " . $url) or die("Didn't  cc work");
    }
...
function remove_from_cart($product){
    ...
    $url = "http://" . $_SERVER['HTTP_HOST'] . "/cart.php?checkout=1";
    header("Location: " . $url) or die("Didn't work");
...
function register_display($session) {
    if (isset($session['valid']['name']) && $session['valid']['name'] == 'name_error' ) {
        $register_display =  '<form name="register" action="cart.php?new_user=1" method="POST">
...
function user_cred($query=array()) {
    ...
    if(isset($_GET['new_user']) && $_GET['new_user'] ==1){
        ...
        elseif (($name_test == '' || $name_test == null) && isset($_POST['email'])) {
            ...
            $url = "http://" . $_SERVER['HTTP_HOST'] . "/cart.php?register_new=1";
